...
"""
    ...
    # db_utils.py - Simplified version for dynamic generation
    db_utils_content = [
        ...
        '    def __init__(self, spark, catalog_name, schema_name, table_name=None):',
        '        self.spark = spark',
        '        self.catalog_name = catalog_name',
        '        self.schema_name = schema_name',
        '        self.table_name = table_name',
        '        self.full_schema_path = "{0}.{1}".format(catalog_name, schema_name)',
        '        self._ensure_catalog_and_schema()',
        '',
        '    def _ensure_catalog_and_schema(self):',
        '        try:',
        '            if self.catalog_name.lower() != "hive_metastore":',
        '                self.spark.sql("CREATE CATALOG IF NOT EXISTS {0}".format(self.catalog_name))',
        '            self.spark.sql("CREATE SCHEMA IF NOT EXISTS {0}".format(self.full_schema_path))',
        '            print("Schema verificado/criado: {0}".format(self.full_schema_path))',
        '        except Exception as e:',
        '            print("Erro ao verificar/criar schema {0}: {1}".format(self.full_schema_path, str(e)))',
        '            raise',
        '',
        ...
        '        full_table_name = "{0}.{1}".format(self.full_schema_path, table_to_check)',
        '        try:',
        '            self.spark.sql("DESCRIBE TABLE {0}".format(full_table_name))',
        '            return True',
        '        except Exception as e:',
        ...
        '            raise',
        '',
        '    def write_dataframe(self, df, table_name=None, mode="append", merge_schema=False, overwrite_schema=False, partition_by=None):',
        '        target_table = table_name or self.table_name',
        '        if not target_table:',
        '            raise ValueError("No table name provided and no default table_name set")',
        '        full_table_name = "{0}.{1}".format(self.full_schema_path, target_table)',
        '        print("Preparando para escrever na tabela: {0}".format(full_table_name))',
        '        # Add metadata columns if they dont exist',
        '        if "ingestion_timestamp" not in df.columns:',
        '            df = df.withColumn("ingestion_timestamp", current_timestamp())',
        '        if "pipeline_run_id" not in df.columns:',
        '            df = df.withColumn("pipeline_run_id", lit("run_{0}".format(int(datetime.now().timestamp()))))',
        '        # Create table if it doesnt exist',
        '        if not self.table_exists(target_table):',
        '            print("Tabela {0} não existe. Criando...".format(full_table_name))',
        '            try:',
        '                # Build CREATE TABLE statement',
        '                create_parts = ["CREATE TABLE {0} (".format(full_table_name)]',
        '                fields = []',
        '                for field in df.schema.fields:',
        '                    fields.append("  {0} {1}".format(field.name, field.dataType.simpleString()))',
        '                create_parts.append(",\n".join(fields))',
        '                create_parts.append(")")',
        '                if partition_by:',
        '                    create_parts.append("PARTITIONED BY ({0})".format(", ".join(partition_by)))',
        '                create_parts.append("USING DELTA")',
        '                create_stmt = "\n".join(create_parts)',
        '                print("Executando: {0}".format(create_stmt))',
        '                self.spark.sql(create_stmt)',
        '                print("Tabela {0} criada com sucesso!".format(full_table_name))',
        '            except Exception as e:',
        '                print("Erro ao criar tabela {0}: {1}".format(full_table_name, str(e)))',
        '                raise',
        '        # Configure writer',
        ...
        '        try:',
        '            print("Escrevendo {0} registros em {1}...".format(df.count(), full_table_name))',
        '            writer.mode(mode).saveAsTable(full_table_name)',
        '            print("Dados escritos com sucesso em {0}".format(full_table_name))',
        '        except Exception as e:',
        '            error_msg = str(e).lower()',
        '            print("Erro ao escrever na tabela {0}: {1}".format(full_table_name, error_msg))',
        ...
        '                print("Tabela não encontrada. Tentando recriar a tabela...")',
        '                self.spark.sql("DROP TABLE IF EXISTS {0}".format(full_table_name))',
